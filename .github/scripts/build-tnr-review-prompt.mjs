...
const authBlock =
  brokerUrl && brokerToken
    ? [
        ...
        `  -d '{"users":[{"key":"player1","level":100,"rank":"JONIN","villageName":"Konoki"}]}'`,
        "```",
        "",
        "The response will be JSON:",
        '```json',
        '{ "success": true, "users": [{ "key": "player1", "userId": "...", "username": "...", "email": "...", "password": "...", "level": 100, "rank": "JONIN", "villageId": "...", "villageName": "Konoki", "isBanned": false }], "testingToken": "..." }',
        "```",
        "",
        "After provisioning, sign in via Clerk's sign-in API by running:",
        "```",
        `curl -s -X POST "https://talented-kit-66.clerk.accounts.dev/v1/client/sign_ins" \\`,
        `  -H "Content-Type: application/json" \\`,
        `  -H "Authorization: Bearer <testingToken>" \\`,
        `  -d '{"identifier":"<email>","password":"<password>"}'`,
        "```",
        "",
        "Then use the returned session cookie/token in Playwright via `browser_run_code` to set cookies, or simply validate that the broker returns success and the user was provisioned correctly.",
        "",
