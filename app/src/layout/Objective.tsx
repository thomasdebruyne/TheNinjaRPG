...
export const Reward: React.FC<RewardProps> = (props) => {
  ...
  if (info?.reward_anbupoints) {
    rewards += `${rewards ? ", " : ""} ${info.reward_anbupoints} Anbu Points`;
  }
  if (info?.reward_exp) {
  ...
  if (info?.reward_skillpoints) {
    rewards += `${rewards ? ", " : ""} ${info.reward_skillpoints} Skill Points`;
  }
